...
entry_t* lookup(symbol_t table, char* name) {
    ...
    // para associ�-lo a um n�mero inteiro
    ...
    if(table.entries[hash] != NULL) {
        ...
        // se n�o for a "entry" que estamos procurando (o "name" � diferente) e ainda existe um nodo seguinte...
...
int insert(symbol_t* table, entry_t* entry) {
    ...
    // inicia a lista encadeada com este elemento
        ...
        // se a lista encadeada j� existe, adiciona o elemento no inicio:
 ...
 *
 * Uma entrada individual da tabela de s�mbolos � transformada em uma string. Declarada aqui para ser chamada pelo print_file_table
 *
...
int print_file_table(FILE* out, symbol_t table) {
    int entry_count = 0;
    // varre a tabela de s�mbolos
    int x = 0;
    for(x = 0; x < PRIME; x++) {
    // onde houver ponteiro, imprime o conte�do da entrada
        if(table.entries[x] != NULL) {
            char tmp_string[500];
